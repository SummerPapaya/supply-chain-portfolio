...
除了报告本身，网站还包含一个**每日供应链雷达**（交叉验证的新闻、研究与趋势），以及一个衍生应用——[VeloCortex Maritime](#velocortex)——将研究发现转化为一个可运行的集装箱追踪指挥中心。

## ✨ 核心特性

- 📡 **实时雷达** — 每日更新、经过交叉验证的情报：覆盖运价、关税与供应链事件——每条信息都链接到原始来源。
...
├── radar-data.json         # 每日雷达数据（内嵌快照 + 可覆盖源）
├── reports/
...
板块还会抓取两个中文网页源——**雨果网**（跨境电商/关税/物流）与**罗戈网**（物流与供应链研究）——它们不提供公开 RSS，生成器用标准库做轻量 HTML 抓取，每栏至少保留一条中文原生新闻，链接均指向原文出处。
